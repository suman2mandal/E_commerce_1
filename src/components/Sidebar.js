...
const Sidebar = () => {
  ...
  const {cart,clearCart,removeFromCart,totalItems,totalAmount} = useContext(CartContext);
  return <div className={`${isOpen?'right-0':'-right-full'} w-full bg-white fixed top-0 h-full shadow-2xl md:w-[35vw] xl:max-w-[30vw]  transition-all duration-300 z-20 px-4 lg:px-[35px]`}>
    ...
    <div className={'flex flex-col gap-y-2 h-[400px] lg:h-[440px] overflow-y-auto overflow-x-hidden scrollbar-hide border-b'}>{cart.map((item)=>{
    ...
    <div className={' flex flex-col gap-y-3 py-4 mt-4'}>
      <div className='flex w-full justify-between items-center'>
        <div className={'uppercase font-semibold'}>
          <span className={'mr-2'}>Total:</span>{`$ ${parseFloat(totalAmount).toFixed(2)}`}
        </div>
